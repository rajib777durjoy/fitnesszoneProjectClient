...
const ForumPost = () => {
    ...
    return (
        <div className='w-[100%] mx-auto bg-slate-50 min-h-screen py-10'>
            ...
            {
                slices?.map(item => <div className='mb-10  bg-slate-200 rounded-lg ' key={item?._id}>
                    ...
                    <h3 className='t my-4 ms-4 text-xl font-semibold'><span className='text-xs font-light mx-2'>{item?.details.slice(0,100)}</span></h3>
                    <h1 className='ms-4 md:ms-2 text-xs my-2'>Vote:{item?.Vote}</h1>
                </div>)
            }
            </div>
